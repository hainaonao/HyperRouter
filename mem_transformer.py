...
class CustomizedMoEPositionwiseFF(FMoETransformerMLP):
    def __init__(self, d_model, d_inner, dropout, pre_lnorm=False, moe_num_expert=64, moe_top_k=2, gate_name=NaiveGate, hyper_size=None):
        activation = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        super().__init__(num_expert=moe_num_expert, d_model=d_model, d_hidden=d_inner, top_k=moe_top_k,
                activation=activation, gate=gate_name, hyper_size=hyper_size)
    ...
    def __init__(self, n_head, d_model, d_head, d_inner, dropout, **kwargs):
        ...
        if kwargs.get('moe') is False and kwargs.get('dense_drop') is False:
            ...
            self.pos_ff = CustomizedMoEPositionwiseFF(d_model, d_inner, dropout,
                                        ...
                                        gate_name=eval(kwargs.get('gate_name')),hyper_size=kwargs.get('hyper_size'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyper_size', type=int, default=None, help='')
