# coding: utf-8
"""
Baseline training script using CustomNaiveGate_Balance on WikiText-103.
Same model architecture & config as train_free_loss.py (Medium Transformer-XL),
but uses standard MoE with auxiliary balancing loss instead of BalancingLossFreeGate.

This serves as a baseline for comparison with the loss-free approach.

Usage:
    python train_base_gate.py \
        --cuda \
        --data /path/to/wt103 \
        --dataset wt103
"""
...
parser.add_argument('--work_dir', default='LM-TFM-BaseGate', type=str,
...
logging = create_exp_dir(
    args.work_dir,
    scripts_to_save=['train_base_gate.py', 'mem_transformer.py'],
    debug=args.debug,
)
        ...
        if train_step % 1000 == 999:
            ...
            if prev_routing_decisions is not None:
                ...
                if fluctuations:
                    ...
                    logging('| Routing Fluctuation at step {:>8d} '
